...
# --- Open-loop simulation with constant dilution u_D(t) = 0.12 h^-1 ---

# Time grid for NOA (slightly coarser than for pure simulation):
T_final = 48.0  # hours
dt = 0.1        # hours
...
# --- Quick sanity check of the outputs B, N, W ---
...
for i, name in enumerate(MEASUREMENT_NAMES):
    plot_tme(t_sim, Y[:, i], measured=None, estimated=None, ax=axes[i], label_var=name)

axes[-1].set_xlabel("Time [h]")
fig.tight_layout()
plt.show()

# --- Build PyBounds simulator and empirical observability matrix ---
...
# --- Plot the minimum error variance per state over time ---
...
ax.set_title("Sliding Fisher observability - u_D(t) = 0.12 h$^{-1}$")
ax.legend()
fig.tight_layout()
plt.show()
