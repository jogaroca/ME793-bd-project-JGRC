# 02_bd_nonlinear_observability.ipynb
...
ROOT = os.path.dirname(os.path.dirname(os.getcwd())) if 'notebooks' in os.getcwd() else os.getcwd()
SRC_PATH = os.path.join(ROOT, "src")
if SRC_PATH not in sys.path:
    sys.path.append(SRC_PATH)

import bd  # this uses the __init__.py we created
...
# Helper plotting functions for observability results
...
def plot_diag_Finv_comparison(results_dict):
    ...
    for k, (name, (_, Finv)) in enumerate(results_dict.items()):
        ...
        plt.bar(x + k * width, np.log10(diag_vals),
                width=width, label=name)

    plt.xticks(x + 0.4, state_names)
    plt.ylabel("log10(min error variance)")
    plt.title("Empirical observability comparison")
    plt.legend()
    plt.tight_layout()
    plt.show()
...
for name, ufun in motifs.items():
    ...
    print(f"  min eigenvalue(W_o) = {np.linalg.eigvalsh(W_o).min():.3e}")
    print(f"  max eigenvalue(W_o) = {np.linalg.eigvalsh(W_o).max():.3e}")
# Example heatmaps for each motif
for name, (W_o, Finv) in results.items():
    plot_Finv_heatmap(Finv, title=f"Finv, motif = {name}")

# Global comparison using diagonal entries of Finv
...
summary_df_pivot = summary_df.pivot(index="state", columns="motif", values="log10_Finv_diag")
summary_df_pivot
