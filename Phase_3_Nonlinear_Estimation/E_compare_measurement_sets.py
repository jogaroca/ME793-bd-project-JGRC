"""
Phase 3 - (Recommended) Compare measurement sets at fixed motif
...
from Phase_3_Nonlinear_Estimation.config_phase3 import default_config, meas_file, results_dir, truth_file
...
def main() -> None:
    ...
    for meas_opt in meas_opts:
        ...
        rmsZ = float(rms_states[0])

        B_true = _B_from_x(x_true)
        B_hat = _B_from_x(x_hat)
        rmsB = float(_rms(B_hat - B_true))

        rows.append({"motif": motif, "meas_opt": meas_opt, "RMS_Z": rmsZ, "RMS_B": rmsB})

    # Save CSV
    csv_path = out_metrics / f"ekf_compare_measopts_{motif}.csv"
    header = ["motif", "meas_opt", "RMS_Z", "RMS_B"]
